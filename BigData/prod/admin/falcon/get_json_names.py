...
class GetJsonNames():
    ...
    logger = logging.getLogger("GetJsonNames")
    ...
    def replace_characters(self, rval):
        ...
        for ii in range(len(rval)):
            ...
            elif self.notgood.search(parts[0]):
    ...
    def remove_comments(self, parts):
        """
        Remove all comments, where the comment is only " -- DATATYPE".
        The comment must be only a single word, no spaces.

        @param parts: Hive column definitions, split on comma
        @type parts: array of strings
        @return: Definitions without comments, keeping the uncommented data type
        @rtype: array of strings
        """
        ...
        for ii in range(len(parts)):
            ...
            if parts[ii].startswith(' -- '):
                ...
                rval.append(parts[ii][0:pos + 1].strip())
            else:
    ...
    def run(self, arg):
        ...
        if len(parts) == 1 and len(parts[0]) == 0:
            return ""
        rval = self.remove_comments(parts)
...
if __name__ == '__main__':
    ...
    gjn = GetJsonNames(mapfile=options.mapfile, tab=options.tab, debug=options.debug)
